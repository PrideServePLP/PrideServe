"use client";

import {
  useServiceTasks,
  useStudentDirectory,
  useTaskViewer,
} from "@/lib/tasks/useTasks";
import { tasksCreatedBy } from "@/lib/tasks/visibility";
import TaskCard from "@/components/tasks/TaskCard";

export default function PostedRequests() {
  const tasks = useServiceTasks();
  const students = useStudentDirectory();
  const viewer = useTaskViewer();
  const mine = tasksCreatedBy(tasks, viewer?.id);

  const nameById = new Map(
    students.map((student) => [student.id, student.fullName]),
  );

  return (
    <section className="space-y-3">
      <div>
        <h2 className="text-lg font-semibold text-plp-navy">
          Requests you posted
        </h2>
        <p className="text-sm text-slate-500">
          Private assignments show the student they were sent to.
        </p>
      </div>

      {mine.length === 0 ? (
        <div className="rounded-xl border border-dashed border-plp-slate-border bg-white px-6 py-12 text-center text-sm text-slate-500">
          You have not posted any service requests yet.
        </div>
      ) : (
        <ul className="grid gap-4 md:grid-cols-2">
          {mine.map((task) => (
            <TaskCard
              key={task.id}
              task={task}
              assigneeName={
                task.assignedStudentId
                  ? (nameById.get(task.assignedStudentId) ?? "Assigned student")
                  : null
              }
            />
          ))}
        </ul>
      )}
    </section>
  );
}
